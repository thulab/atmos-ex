...
# ----------------------------------------------------------------------------
# Author: qingxin.feng
# Description: 自动化执行IoTDB API写入测试，结构优化版。
# ------------------------------------------------------------------------------
...
protocol_list=(223)
ts_list=(tempaligned)
api_list=(SESSION_BY_TABLET SESSION_BY_RECORDS SESSION_BY_RECORD JDBC)
...
DBNAME="QA_ATM"                  # 数据库名称
TABLENAME="ex_api_insert_cts"         # 结果表名
TABLENAME_T="ex_api_insert_cts_T"         # 企业版结果表名
...
function backup_test_data() {
    local api_type=$1
    local protocol_id=$2
    local backup_dir="${BUCKUP_PATH}/${api_type}/${commit_date_time}_${commit_id}_${protocol_id}"
...
function test_operation() {
    ...
    check_benchmark_pid
    check_iotdb_pid
    backup_test_data "${api_type}" "${protocol_class_input}"
}
