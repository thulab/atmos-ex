...
#登录用户名
ACCOUNT=root
TEST_TYPE="${TEST_TYPE:-native_api_test}"
#初始环境存放路径
export HTTP_PROXY="${HTTP_PROXY:-http://172.20.31.15:7890}"
  	...
  	return 0
		...
		do
			...
			then
				...
				log "第"$j"次尝试编译IoTDB失败"
				if [ $j -eq 5 ]; then
					log "已经尝试5次编译IoTDB失败，写入负值测试结果！"
					return 1
					break
				fi
			fi
...
test_java_native_api_test() { # 测试Java原生接口
	...
	if [ $? -eq 0 ]
	then
		log "编译Java原生接口工具完成，准备开始测试！"
	else
		log "编译Java原生接口工具失败，写入负值测试结果！"
		tests_num=-2
		errors_num=-2
		failures_num=-2
		skipped_num=-2
...
test_cpp_native_api_test() {
	...
	if [ $? -eq 0 ]; then
		log "编译C++客户端完成，准备开始测试！"
	else
		log "编译C++客户端失败，写入负值测试结果！"
		tests_num=-2
		errors_num=-2
		failures_num=-2
		skipped_num=-2
	...
	if [ $? -eq 0 ]; then
		log "编译Cpp原生接口测试工具完成，准备开始测试！"
	else
		log "编译Cpp原生接口工具失败，写入负值测试结果！"
		tests_num=-3
		errors_num=-3
		failures_num=-3
		skipped_num=-3
...
test_c_native_api_test() {
	...
	# 编译工具
	cd "${TEST_C_TOOL_PATH}" || return 1
	compile=$(timeout 300s bash -c "source /etc/profile && ./compile.sh")
	if [ $? -eq 0 ]; then
		log "编译C原生接口测试工具完成，准备开始测试！"
	else
		log "编译C原生接口工具失败，写入负值测试结果！"
		tests_num=-3
		errors_num=-3
		failures_num=-3
		skipped_num=-3
...
test_python_native_api_test() { # 测试Python原生接口
	...
	if [ $? -eq 0 ]; then
		log "编译Python客户端完成，准备开始测试！"
	else
		log "编译Python客户端失败，写入负值测试结果！"
		tests_num=-2
		errors_num=-2
		failures_num=-2
		skipped_num=-2
...
if [ "${last_cid_iotdb}" != "${commit_id_iotdb}" ]; then # 判断IoTDB代码是否更新
    ...
    bash "${ATMOS_PATH}/script/scenarios/tsfile_api_test.sh" >> "${INIT_PATH}/log_tsfile_api_test" 2>&1 &
else # 没有更新则等待下一轮测试
	log "没有更新，都执行过测试"
	# 调用tsfile测试
	echo "tsfile_api_test" > ${INIT_PATH}/test_type_file
