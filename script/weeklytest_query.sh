...
mv_config_file() { # 移动配置文件
	...
	cp -rf ${ATMOS_PATH}/conf/weeklytest_query/$1 ${BM_PATH}/conf/config.properties
}
test_operation() {
	protocol_class=$1
	#查询测试	
	...
	cp -rf ${DATA_PATH}/${query_data_type[${j}]}/data ${TEST_IOTDB_PATH}/
		for (( i = 0; i < ${#query_list[*]}; i++ ))
			...
			mv ${TEST_IOTDB_PATH}/logs ${TEST_IOTDB_PATH}/logs_${query_list[${i}]}_${m}
			done
			#停止IoTDB程序
			check_iotdb_pid
		done
...
if [ "${commit_id}" = "" ]; then
	...
	result_string=$(mysql -h${MYSQLHOSTNAME} -P${PORT} -u${USERNAME} -p${PASSWORD} ${DBNAME} -e "${update_sql}")
	#清理过期文件 - 当前策略保留4天
	find ${BUCKUP_PATH}/no_overflow -mtime +3 -type d -name "*" -exec rm -rf {} \;
	find ${BUCKUP_PATH}/is_overflow -mtime +3 -type d -name "*" -exec rm -rf {} \;
fi
echo "weeklytest_query" > ${INIT_PATH}/test_type_file
