...
modify_iotdb_config() { # iotdb调整内存，关闭合并
	...
	#修改集群名称
	echo "cluster_name=Apache-IoTDB-2" >> ${TEST_CONFIGNODE_PATH}/conf/iotdb-system.properties
	echo "cluster_name=Apache-IoTDB-2" >> ${TEST_DATANODE_PATH}/conf/iotdb-system.properties
	#添加启动监控功能
...
test_operation() {
	...
	for ((j = 1; j <= 5; j++)); do
		...
		ssh ${ACCOUNT}@${D_IP_list[${j}]} "sudo mv -rf ${TEST_DATANODE_PATH}/dn_dump.hprof ${INIT_PATH}/${ts_type}_${commit_date_time}_${commit_id}_${data_type}_${protocol_class}_dn_dump.hprof"
	done
