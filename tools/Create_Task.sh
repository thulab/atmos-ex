...
if [ ! -d "${REPO_PATH}/${commit_id}/apache-iotdb" ]; then
	...
	if  [[ "$day_of_week" -eq 6 ]] || [[ "$day_of_week" -eq 3 ]]; then
		...
		update_sql="update ${TABLENAME} set windows_test = '${str_type}',weeklytest_insert = '${str_type}',weeklytest_query = '${str_type}',config_insert = '${str_type}',cluster_insert = '${str_type}',cluster_insert_2 = '${str_type}',restart_db = '${str_type}',count_ts = '${str_type}' where commit_id = '${commit_id_new}'"
