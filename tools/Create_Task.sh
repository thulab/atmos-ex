...
if [ ! -d "${REPO_PATH}/${commit_id}/apache-iotdb" ]; then
	...
	sleep 10
	str_type='retest'
	insert_sql="insert into ${TABLENAME} (commit_date_time,commit_id,author,se_insert,unse_insert,se_query,unse_query,compaction,api_insert,ts_performance,insert_records,pipe_test,last_cache_query,api_insert_cts,se_query_test,pipe_test_win,windows_test,config_insert,weeklytest_insert,weeklytest_query,restart_db,count_ts,sql_coverage,routine_test,benchants,helishi_test,cluster_insert,cluster_insert_2,remark) values(${commit_date_time},'${commit_id_new}','${author}','${str_type}','${str_type}','${str_type}','${str_type}','${str_type}','${str_type}','${str_type}','${str_type}','${str_type}','${str_type}','${str_type}','${str_type}','${str_type}','${str_type}','NoNeed','NoNeed','NoNeed','NoNeed','NoNeed','NoNeed','NoNeed','NoNeed','NoNeed','NoNeed','NoNeed','TimechoDB')"
